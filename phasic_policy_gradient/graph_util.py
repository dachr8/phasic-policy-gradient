...
def plot_data_mean_std(ax, data_y, color=(57, 106, 177), data_x=None, x_scale=1, smoothing=0, first_valid=0,
                       label=None):
...
def plot_experiment(csv_file_groups, single_env_name, titles=None, normalization_ranges=None, key_name='eprewmean',
                    labels=None, bbox_to_anchor=(.5, 0, .5, 1), **kwargs):
    num_envs = len(ENV_NAMES)
    will_normalize_and_reduce = normalization_ranges is not None

    if will_normalize_and_reduce or single_env_name:
        num_visible_plots = 1
    ...
    for curve_idx, csv_file_group in enumerate(csv_file_groups):
        if single_env_name:
            ...
            csv_files = [f"tmp/{dir_name}/progress-{env_name}.csv" for dir_name in csv_file_group]

            raw_data = np.array([read_csv(file, key_name) for file in csv_files])
            plot_values(ax, raw_data, title=env_name, color=color, label=label, **kwargs)
        else:
            all_values = []
